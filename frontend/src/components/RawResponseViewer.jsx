    ...
    <div className="space-y-5 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-purple-50/50 p-3.5 rounded-2xl border border-purple-100">
          ...
          <span className="text-xs font-extrabold text-purple-800 font-mono mt-1 block flex items-center gap-1.5">
            <Clock className="w-3.5 h-3.5 text-purple-600" /> {processingTime || 0} ms
          </span>
        </div>
        ...
        <div className="bg-purple-50/40 p-4 rounded-2xl border border-purple-100 space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5 font-mono text-[11px]">
            {missingFields.map((f, i) => (
              <span key={i} className="bg-white text-slate-600 border border-purple-200 px-2 py-0.5 rounded-md">
                {f}
              </span>
            ))}
          </div>
      ...
      {/* Raw Payload Text */}
      <div className="bg-purple-50/40 rounded-2xl border border-purple-100 p-4 font-mono text-xs text-slate-800 overflow-x-auto max-h-[500px] shadow-inner">
